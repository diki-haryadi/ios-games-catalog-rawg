...
class ImageUtility {
    ...
    func loadImage(from urlString: String?, into imageView: UIImageView, placeholder: UIImage? = nil) {
        ...
        guard let urlString = urlString, !urlString.isEmpty else {
            print("⚠️ ImageUtility: Empty or nil URL string provided")
            return
        }
        ...
        if let directURL = URL(string: urlString) {
            ...
            print("⚠️ ImageUtility: URL required encoding: \(urlString) → \(encodedUrlString)")
            imageURL = encodedUrl
        }
        ...
        let task = session.dataTask(with: imageURL) { [weak self] data, response, error in
            ...
            DispatchQueue.main.async {
                ...
                if imageView.accessibilityIdentifier != requestID {
                    print("⚠️ ImageView was recycled for a different request, aborting image update")
                    return
                }
